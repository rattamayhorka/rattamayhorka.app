    ...
    <div className="space-y-8 text-left">
      ...
      <div className="space-y-10 relative pl-4">
        {fechasOrdenadas.length > 0 ? (
          fechasOrdenadas.map(fecha => {
            ...
            if (diffDays === 0) { 
              tituloFecha = "HOY"; 
              /* bgDia = "bg-red-950 text-red-400 border-red-900"; colorDia = "text-red-400"; */
              bgDia = "bg-theme-casa/20 text-theme-casa border-theme-casa"; 
              colorDia = "text-theme-casa"; 
            } else if (diffDays === 1) { 
              tituloFecha = "MAÑANA"; 
              /* bgDia = "bg-amber-950 text-amber-400 border-amber-900"; colorDia = "text-amber-400"; */
              ...
              <div key={fecha} className="relative pl-8 border-l-2 border-theme-border/40">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {items.map((item, idx) => {
                    ...
                    // MODIFICACIÓN: Color de fondo y borde adaptado al tipo
      ...
      {modalAbierto && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-theme-bg rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-theme-border">
            ...
            <form onSubmit={ejecutarGuardarReunion} className="p-6 space-y-4">
              ...
              <div>
                ...
                <label className="block text-[9px] font-black uppercase text-theme-text/60 mb-1">Tipo de Evento</label>
              ...
              <div>
                ...
                <label className="block text-[9px] font-black uppercase text-theme-text/60 mb-1">Nombre del Comité / Evento</label>
                ...
                <input type="text" name="comite" required value={formData.comite} onChange={manejarCambioInput} className="w-full bg-theme-bg border border-theme-border rounded-lg p-2.5 text-xs text-theme-text focus:border-theme-accent outline-none uppercase font-bold" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" name="fecha" required value={formData.fecha} onChange={manejarCambioInput} className="w-full bg-theme-bg border border-theme-border rounded-lg p-2.5 text-xs text-theme-text focus:border-theme-accent outline-none font-bold" />
                </div>
                <div>
                  ...
                  <input type="time" name="hora" required value={formData.hora} onChange={manejarCambioInput} className="w-full bg-theme-bg border border-theme-border rounded-lg p-2.5 text-xs text-theme-text focus:border-theme-accent outline-none font-bold" />
                </div>
              </div>
              <div>
                {/* <label className="block text-[9px] font-black uppercase text-zinc-400 mb-1">Lugar o Link de Meet</label> */}
                <label className="block text-[9px] font-black uppercase text-theme-text/60 mb-1">Lugar o Link de Meet</label>
                ...
                <input type="text" name="lugar" required value={formData.lugar} onChange={manejarCambioInput} className="w-full bg-theme-bg border border-theme-border rounded-lg p-2.5 text-xs text-theme-text focus:border-theme-accent outline-none uppercase font-bold" />
              </div>
              ...
              <button type="submit" disabled={guardando} className="w-full bg-theme-accent text-theme-bg font-black uppercase text-[10px] py-3 rounded-lg hover:opacity-90 transition-colors cursor-pointer disabled:opacity-50">
      ...
      {modalEliminar && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-theme-bg rounded-2xl shadow-2xl w-full max-w-xs overflow-hidden border border-theme-border border-t-8 border-t-theme-casa text-center p-6">
            ...
            <h4 className="text-sm font-black text-theme-text uppercase tracking-tighter italic mb-2">¿Eliminar Convocatoria?</h4>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => { setModalEliminar(false); setReunionSeleccionada(null); }} className="flex-1 text-[10px] font-black uppercase text-theme-text/60 hover:text-theme-text cursor-pointer">Cancelar</button>
              <button 
